...
CUERPO_REPORTE = rmarkdown::render("3) OUTPUT//1) CORREO//Cuerpo-Reporte.Rmd",
                                # output_dir = "3) Html (Output)",
                                output_file = "Cuerpo-Reporte.html")
...
msg = gm_mime() %>% 
  ...
  gm_text_body("Estimado equipo, mediante el presente correo se adjunta la versión de prueba del primer reporte estad{istico de los actos administrativos emitidos y pendientes por parte de la DFAI.\n\nCabe resaltar que esta es una versiónd e prueba, por lo cual los datos deben tomarse con prudencia, sin embargo, del primer testeo, se observa que la herramienta presenta un error de alrededor del 3%.\n\nSaludos Cordiales.\n\n-----------------------------\nLuis Felipe") %>%
